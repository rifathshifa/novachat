...
# ─────────────────────────────────────────────────────────
# sid → user_id lookup table
# This is the reliable way to identify users in socket events
# when using eventlet (Flask session is NOT persistent across
# eventlet greenlets/socket events reliably).
# ─────────────────────────────────────────────────────────
_sid_to_user_id = {}


def get_user_id_from_sid(sid):
    """Returns the authenticated user_id for a given socket SID, or None."""
    return _sid_to_user_id.get(sid)
...
def register_socket_events(app):
    ...
    def handle_connect(auth=None):
        ...
        try:
            ...
            # ── Store in sid map (reliable across eventlet greenlets) ──
            _sid_to_user_id[sid] = user_id

            # Also set in Flask session as a secondary fallback
    ...
    def handle_disconnect():
        ...
        # ── Use sid map for reliable user lookup ──
